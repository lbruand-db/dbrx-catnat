...
Subcommands:
  ...
  catnat bench              Run the SPEC §7 P1 performance benchmark set.
"""
...
def bench(
    ...
    no_warmup: bool = typer.Option(
        False, "--no-warmup", help="Skip the warmup run (first-call latency counts)."
    ),
    out: Path = typer.Option(
        None, "--out", help="Write a markdown report to this path (e.g. SPECS/BENCHMARKS.md)."
    ),
) -> None:
    """Time the gold-layer access patterns and report min/median/p95 latencies."""
    from rich.table import Table

    from catnat.bench import run as run_bench

    console.print(
        f"[bold]Running benchmarks[/bold] — {n_runs} timed runs per query, "
        f"warmup={'off' if no_warmup else 'on'}, catalog={CONFIG.catalog}"
    )
    results = run_bench(n_runs=n_runs, warmup=not no_warmup)

    table = Table(title="P1 benchmark results")
    table.add_column("Query", style="cyan", no_wrap=True)
    table.add_column("Description", overflow="fold")
    for col in ("min", "median", "p95", "max"):
        table.add_column(f"{col} (ms)", justify="right")
    for r in results:
        table.add_row(
            r.name,
            r.description,
    ...
    if out:
        ...
        target_ms = 1000  # SPEC §7 P1: <1 s on a Small Serverless SQL WH.
        n_pass = sum(1 for r in results if r.median_ms < target_ms)
        lines = [
            "# P1 benchmarks",
            "",
            f"_Generated by `catnat bench --out {out.name}` "
            f"at {datetime.now(UTC).isoformat(timespec='seconds')}._",
            "",
            "**Target** (SPEC §7 P1): every gold-layer point-in-polygon at portfolio "
            f"scale runs in **< {target_ms} ms** on a Small Serverless SQL Warehouse.",
            "",
            f"**Status**: **{n_pass} / {len(results)}** queries pass the target on median latency.",
            "",
            f"Catalog: `{CONFIG.catalog}`. Runs per query: {n_runs} "
            f"({'warmup discarded' if not no_warmup else 'no warmup'}).",
            "",
            "| Query | Description | min | median | p95 | max | ✓ |",
            "|---|---|---:|---:|---:|---:|:---:|",
        ]
        for r in results:
            ok = "✅" if r.median_ms < target_ms else "❌"
            lines.append(
                f"| `{r.name}` | {r.description} | "
                f"{r.min_ms:.0f} ms | {r.median_ms:.0f} ms | "
                f"{r.p95_ms:.0f} ms | {r.max_ms:.0f} ms | {ok} |"
            )
        lines.append("")
        lines.append(
            "Re-run with `uv run catnat bench --out SPECS/BENCHMARKS.md`. "
            "Query SQL lives in `src/catnat/bench.py`."
        )
        out.write_text("\n".join(lines) + "\n", encoding="utf-8")
        console.print(f"\n[bold green]✓[/bold green] wrote {out}")
