...
| **P2 — Databricks App scaffold** 🟡 in progress | ~2 days | Scaffolded with [apx](https://github.com/databricks-solutions/apx) under `packages/app/` (uv workspace member). React 19 + Vite + TypeScript (strict), Biome for lint/format, Vitest + RTL for tests. Three-pane layout: Leaflet (operational, imperative — direct map handle for the future MCP `add_layer` / `style_layer`), Kepler.gl (stub — package installed, real wiring in Phase 2.5), chat (input + history shell, agent wiring is Phase 4). `/api/layers` reads `catnat_silver.layer_index` via OBO. **17 tests** total: 14 frontend (Vitest), 3 backend (pytest+FastAPI TestClient) — all workspace-free, all green in CI. |
